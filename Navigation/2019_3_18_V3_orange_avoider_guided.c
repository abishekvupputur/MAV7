...
// define settings
float oag_color_count_frac = 0.14f;       // obstacle detection threshold as a fraction of total of image
float oag_floor_count_frac = 0.075f;       // floor detection threshold as a fraction of total of image
...
void orange_avoider_guided_periodic(void)
{
  ...
  if(color_count < color_count_threshold){
    ...
    obstacle_free_confidence -= 3.5;  // be more cautious with positive obstacle detections
  }

  // bound obstacle_free_confidence
  Bound(obstacle_free_confidence, 0, max_trajectory_confidence);

  float speed_sp = fminf(oag_max_speed, 0.14f * obstacle_free_confidence);

  switch (navigation_state){
    case SAFE:
      if (floor_count < floor_count_threshold || fabsf(floor_centroid_frac) > 0.12){
        navigation_state = OUT_OF_BOUNDS;
      } else if (obstacle_free_confidence <= 4.5){
        navigation_state = OBSTACLE_FOUND;
      } else {
	// randomly select new search direction, self added
      	chooseRandomIncrementAvoidance();
    ...
    case SEARCH_FOR_SAFE_HEADING:
      ...
      if (obstacle_free_confidence >= 1.0){
	guidance_h_set_guided_body_vel(speed_sp, 0);
        guidance_h_set_guided_heading(stateGetNedToBodyEulers_f()->psi);
    ...
    case OUT_OF_BOUNDS:
      ...
      guidance_h_set_guided_heading_rate(avoidance_heading_direction * RadOfDeg(45));
